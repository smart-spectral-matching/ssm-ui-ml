package gov.ornl.ssm.ml.ui.data;

import com.vaadin.flow.component.AbstractField.ComponentValueChangeEvent;

import java.util.Arrays;
import java.util.List;

import com.vaadin.flow.component.HasComponents;
import com.vaadin.flow.component.HasValue.ValueChangeListener;
import com.vaadin.flow.component.textfield.NumberField;

import gov.ornl.ssm.ml.ui.components.LabelFeaturesDialog;

/**
 * A feature defined by the ratio of the highest values in two given ranges.
 * 
 * @author Robert Smith
 *
 */
public class PeaksRatioTwoRangeFeature implements Feature {

	/**
	 * The dialog that will display this feature
	 */
	private LabelFeaturesDialog dialog;
	
	/**
	 * The filter whose feature this represents
	 */
	private Filter filter;
	
	/**
	 * List of models this feature will be used to extract data from.
	 */
	private List<Model> models;

	/**
	 * The default constructor.
	 * 
	 * @param filter
	 */
	public PeaksRatioTwoRangeFeature(Filter filter, List<Model> models, LabelFeaturesDialog dialog) {
		this.filter = filter;
		this.models = models;
		this.dialog = dialog;
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see
	 * gov.ornl.ssm.ml.ui.data.Feature#draw(com.vaadin.flow.component.HasComponents,
	 * int)
	 */
	@Override
	public void draw(HasComponents layout, int index) {

		// A field for the range start
		NumberField start1Field = new NumberField();
		start1Field.setLabel("Range 1 Start");
		start1Field.setValue(0d);

		// A field for the range end
		NumberField end1Field = new NumberField();
		end1Field.setLabel("Range 1 End");
		end1Field.setValue(1d);
		
		// A field for the range start
		NumberField start2Field = new NumberField();
		start2Field.setLabel("Range 2 Start");
		start2Field.setValue(2d);

		// A field for the range end
		NumberField end2Field = new NumberField();
		end2Field.setLabel("Range 2 End");
		end2Field.setValue(3d);
		
		//Initialize the input fields if the filter already has a definition
		for(Object feature : filter.getFeatures().get(index)) {
			if(feature instanceof String && ((String) feature).startsWith("SSM:XY:axis:PEAKS-RATIO-TWO-RANGES-")) {
				String[] tokens = ((String) feature).split("-");
				
				start1Field.setValue(Double.valueOf(tokens[4]));
				end1Field.setValue(Double.valueOf(tokens[5]));
				start2Field.setValue(Double.valueOf(tokens[6]));
				end2Field.setValue(Double.valueOf(tokens[7]));
				
				break;
			}
		}
		
		Feature finalThis = this;

		// A listener for both the controls
		ValueChangeListener<ComponentValueChangeEvent<NumberField, Double>> listener = new ValueChangeListener<ComponentValueChangeEvent<NumberField, Double>>() {

			/*
			 * (non-Javadoc)
			 * 
			 * @see com.vaadin.flow.component.HasValue.ValueChangeListener#valueChanged(com.
			 * vaadin.flow.component.HasValue.ValueChangeEvent)
			 */
			@Override
			public void valueChanged(ComponentValueChangeEvent<NumberField, Double> arg0) {

				// Set the range based on the controls
				filter.getFeatures().set(index,
						Arrays.asList("scidata", "dataseries",
								"SSM:XY:axis:PEAKS-RATIO-TWO-RANGES-" + start1Field.getValue() + "-" + end1Field.getValue() + "-" + start2Field.getValue() + "-" + end2Field.getValue(),
								"parameter", "numericValueArray", 0, "numberArray"));
				
				//Update all models' validity
				for(Model model : models) {
					
					//Get the x axis
					List<Double> axis = model.getScidata().getDataseries().get(0).getxAxis().getParameter().getNumericValueArray().get(0).getNumberArray();
					
					//If the defined range is entirely outside the axis's range, this model is invalid.
					if(axis.get(0) > end1Field.getValue() || axis.get(axis.size() - 1) < start1Field.getValue()) {
						model.getInvalidFeatures().add(finalThis);
					} else {
						model.getInvalidFeatures().remove(finalThis);
					}
				}
				
				dialog.refreshGrid();
			}

		};

		start1Field.addValueChangeListener(listener);
		end1Field.addValueChangeListener(listener);
		start2Field.addValueChangeListener(listener);
		end2Field.addValueChangeListener(listener);

		// Layout
		layout.add(start1Field);
		layout.add(end1Field);
		layout.add(start2Field);
		layout.add(end2Field);
	}
	
	/**
	 * Get a human readable name for this type of feature.
	 * 
	 * @return
	 */
	public static String getName() {
		return "Ratio Between Peaks in Two Ranges";
	}
}
